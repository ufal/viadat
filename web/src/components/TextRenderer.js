...
let Paragraph = (props) => {
    ...
    for (let p of parts) {
        if (offset !== p.pos) {
            ...
            tmp_stack.sort((a, b) => props.segments[a].zlevel - props.segments[b].zlevel);
    ...
    if (offset !== text.length) {
        ...
        let id = id_prefix + offset;
        output.push(<span key={id} id={id}>{t}</span>);
    }
    return output;
}

let Speech = (props) => {
    return (<div className="border" style={{margin: "1em"}}>
        <p><b>{props.speech.speaker}</b></p>
        {props.speech.paragraphs.map(p => (
            <Paragraph key={p.id} p={p} renderer={props.renderer} segments={props.segments[p.id]}/>))}
    </div>)
}

let SelectedInfo = (props) => {
    return (<span style={{margin: "1em"}}>{formatTime(props.info.begin)} - {formatTime(props.info.end)}</span>)
}

let AudioInfo = (props) => {
    return (<span style={{margin: "1em"}}>Playing: {formatTime(props.info.position)} / {formatTime(props.info.length)}</span>)
}

let NewLabelDialog = (props) => {
    return (
        <Modal show={props.show} onHide={() => {props.renderer.closeDialog();}}>
        <Modal.Header closeButton>
            <Modal.Title>Select label</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            <LabelTree onLabelSelect={(node) => {
                let instance = {
                    label: node._id,
...
let Header = (props) => {
    return (
        <Table striped bordered condensed hover>
            <tbody>
            {props.document.properties.map((p, i) => <tr key={i}><td>{p.name}</td><td>{p.value}</td></tr>)}
            </tbody>
        </Table>
    );
}
...
class TextRenderer extends Component {
    ...
    recompState(state) {
        this.setState(update(state, {segments: {$set: this.computeSegments(state)}}));
    }

    componentDidMount() {
        fetch_document(this.props.transcript).then((document) => {
            this.recompState(update(this.state,
        ...
        fetch_labels().then((data) => {
            ...
            this.recompState(update(this.state, {labelHintLemmas: {$set: lemmas}}));
        })
    }
    ...
    onClick(key) {
        if (this.state.tab === "audio") {
            ...
            if (key) {
                ...
                audioSelection = {p: key.p, sentence: key.sentence, begin: begin, end: end};
            }
            this.recompState(update(this.state,
                {audioSelection: {$set: audioSelection}}));
        }
    ...
    computeSegmentsForP(state, key) {
        if (state.tab === "audio") {
            ...
            let output = state.document.sentences[key].map(s => ({
                from: s.from,
                to: s.to,
                background: audioSelection && audioSelection.p === key && audioSelection.sentence === i ? "#AADD00" : null,
    ...
    play() {
        ...
        let url = SERVER_URL + "audio/" + this.props.transcript.audio.uuid + "/" + begin + "-" + end;
        ...
        this.sound = soundManager.createSound({
            ...
            autoPlay: true,
            whileplaying: () => {
                this.setState(update(this.state, {audioInfo: {$set: {position: this.sound.position / 1000, length: length}}}));
            }
        });
    ...
    onTabSelect(key) {
        this.recompState(update(this.state, {audioInfo: {$set: null},
                                             tab: {$set: key}}
                                        ));
    }
    ...
    render() {
        return (
                ...
                <p>
                <Button disabled={!this.state.audioSelection} onClick={() => this.play()} bsSize="large"><Glyphicon glyph="play" /> Play</Button>

                <Button disabled={!this.state.audioInfo} onClick={() => this.stop()} bsSize="large"><Glyphicon glyph="stop" /> Stop</Button>
                {
                    this.state.selectedInfo && <SelectedInfo info={this.state.selectedInfo}/>
                }
                {
                    this.state.audioInfo && <AudioInfo info={this.state.audioInfo}/>
                }
                </p>
                </Tab>
                <Tab eventKey="labels" title="Labels">
                    <Button onClick={() => this.onNewTag()}>New label</Button>
                    <Checkbox inline onClick={e => this.toggleHint(e.target.checked)} style={{"marginLeft": "1em"}}>Show hint</Checkbox>
                </Tab>
                </Tabs>
                        </div>
                        </div>
                ...
                {this.state.document && this.state.segments &&
                 this.state.document.speeches.map(
                     (s, index) => <Speech renderer={this} key={index} speech={s} segments={this.state.segments}/>)}

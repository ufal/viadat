...
class LabelDialog extends Component {
  ...
  onFromDate = date => {
    ...
    this.setState({
      ...this.state,
      label: { ...this.state.label, from_date: d.toUTCString() }
    });
  };
  ...
  onToDate = date => {
    ...
    this.setState({
      ...this.state,
      label: { ...this.state.label, to_date: d.toUTCString() }
    });
  };
            ...
            <div>
              {this.state.items_and_counts && (
                <Table bordered striped>
                  ...
                  <tbody>
                    {this.state.items_and_counts.map(ic => (
                      <tr key={ic.item._id}>
                        <td>{ic.item.name}</td>
                        <td>{ic.count}</td>
                      </tr>
              ...
              <div style={{ paddingTop: "1em", paddingBottom: "1em" }}>
                ...
                <DatePicker
                  selected={
                    this.state.label.from_date &&
                    moment(this.state.label.from_date)
                  }
                ...
                <DatePicker
                  selected={
                    this.state.label.to_date && moment(this.state.label.to_date)
                  }
              ...
              <div>
                {this.state.label.lemmas &&
                  this.state.label.lemmas.map((lemma, i) => (
                    <span key={i}>
                      <Label
                        style={{ cursor: "pointer" }}
                        onClick={e => this.onLemmaRemove(i)}
                      >
                        {lemma}
                      </Label>{" "}
                    </span>
    ...
    return (
      <div>
        ...
        <LabelTree
          showMap={true}
          ref={r => (this.tree = r)}
          onLabelSelect={label => this.labelSelect(label)}
        />
        {this.state.label && (
          <LabelDialog
            label={this.state.label}
            closeDialog={reload => this.closeDialog(reload)}
          />
        )}
      </div>
    );

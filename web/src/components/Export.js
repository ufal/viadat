...
class Export extends Component {
    ...
    onExport() {
        this.setState(update(this.state, {statusText: {$set: "Exporting ..."}}));
        run_export().then((r) => {
            this.refresh();
            if (r.status === 200) {
                this.setState(update(this.state, {statusText: {$set: "Exported"}}));
            } else {
                this.setState(update(this.state, {statusText: {$set: "Error: " + r.statusText}}));
            }
        }, (e) => {
            this.setState(update(this.state, {statusText: {$set: "Error: " + e}}));
        })
    }

    render() {
        return (<div><h1>Export</h1>
            ...
            <Button onClick={() => this.onExport()}>Run export</Button> {this.state.statusText}
